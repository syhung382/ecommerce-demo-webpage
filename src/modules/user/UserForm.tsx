...
import type { UserFormProps } from "../../utils/interface";
...
import {
  handleUserAddAsync,
  handleUserDetailAsync,
} from "../../api/handle/handleUsers";
...
const UserForm = ({ id, handleSuccess }: UserFormProps) => {
  const {
    control,
    setValue,
    handleSubmit,
  ...
  const [roleAdmin, setRoleAdmin] = useState<number>(RoleAdminEnum.Staff);
  ...
  //change roleAdmin
  const handleChangeRoleAdmin = (value: number) => {
    setRoleAdmin(value);
    setValue("roleAdmin", value);
  };
  ...
  const handleAddAsync = async (value: UserReq) => {
    const res = await dispatch(handleUserAddAsync(value));
    if (res) {
  ...
  const fetchData = async (id: number) => {
    setLoading(true);
    try {
      const res = await dispatch(handleUserDetailAsync(id));
      if (res) {
        if (res.meta.requestStatus === "rejected") {
          toast.error("Connect server error!");
        }
        ...
        <div className="flex flex-row w-full gap-x-4">
          ...
          <Field>
            ...
            <div className="flex flex-row gap-x-4">
              <RadioInput
                name="roleAdmin"
                onChange={() => handleChangeRoleAdmin(RoleAdminEnum.Staff)}
                title="Nhân viên"
                value={RoleAdminEnum.Staff}
                status={roleAdmin}
              ></RadioInput>
              <RadioInput
                name="roleAdmin"
                onChange={() => handleChangeRoleAdmin(RoleAdminEnum.Moderator)}
                title="Quản lý"
                value={RoleAdminEnum.Moderator}
                status={roleAdmin}
              ></RadioInput>
              <RadioInput
                name="roleAdmin"
                onChange={() => handleChangeRoleAdmin(RoleAdminEnum.Admin)}
                title="Quản trị viên"
                value={RoleAdminEnum.Admin}
                status={roleAdmin}
              ></RadioInput>
              <RadioInput
                name="roleAdmin"
                onChange={() => handleChangeRoleAdmin(RoleAdminEnum.SuperAdmin)}
                title="Quản trị viên *"
                value={RoleAdminEnum.SuperAdmin}
                status={roleAdmin}
              ></RadioInput>
            </div>
          </Field>
