...
import type { UserTableProps } from "../../utils/interface";
...
const UserTable = ({ navigate }: UserTableProps) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await dispatch(handleUserDeleteAsync(deleteIds));
      if (res) {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await dispatch(handleUserListAsync(requestParams));
      if (res) {
        if (res.meta.requestStatus === "rejected") {
          toast.error("Connect server error!");
        }
      ...
      <table className="table-auto w-full border-gray-200 rounded-xl ">
        <thead>
          <tr className="bg-blue-100 dark:bg-gray-700 text-left dark:border-b dark:border-gray-400 dark:border-collapse">
            ...
            <th className="px-4 py-2 text-center border-x border-gray-300 dark:border-gray-600 w-[18%]">
              Chức vụ
            </th>
            <th className="px-4 py-2 text-center w-[16%]">Lần cuối</th>
          ...
          {!loadingContent &&
            listData.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-2 text-center">
                  {(item.roleAdmin && getRoleAdmin(item.roleAdmin)) || "--"}
                </td>
                <td className="px-4 py-2 text-center">
                ...
                <td className="px-4 py-2">
                  <div className="flex gap-x-2">
                    ...
                    />
                    {item.roleAdmin !== RoleAdminEnum.SuperAdmin &&
